...
class YazOkulu extends StatefulWidget {
  @override
  State<YazOkulu> createState() => _YazOkuluPageState();
}

class _YazOkuluPageState extends State<YazOkulu> {

  List<String> modelList = ["Bir dosya" , "İki dosya", "Üç dosya", "Dört dosya", "Beş dosya", "Altı dosya",
    "Yedi dosya", "Sekiz dosya", "Dokuz dosya", "On dosya", "On bir dosya", "On iki dosya", "On üç dosya"];
      ...
      body: DefaultTabController(
        ...
        child: Scaffold(
          appBar: AppBar(
            bottom: TabBar(
              tabs: [
                Tab(icon: Icon(Icons.swap_vertical_circle_outlined), text: 'Gelen',),
                Tab(icon: Icon(Icons.check_circle_rounded), text: 'Kabul Edilen',),
                Tab(icon: Icon(Icons.cancel), text: 'Red Edilen',),
              ],
            ),
          ),
        ),
